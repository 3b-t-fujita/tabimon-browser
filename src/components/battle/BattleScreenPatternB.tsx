/**
 * バトル画面 パターンB — 臨場感強化型
 * ワールドカラー背景 + 敵を大フィーチャー + 光るスキルボタン。
 * オートバトルを見ていて楽しく、スキル発動の瞬間が気持ちいい体験を優先する。
 */
'use client';
...
// ── 敵カード（大・立ち絵フォーカス） ─────────────────────────
function EnemyCard({ actor, accent }: { actor: BattleActor; accent: string }) {
  ...
  return (
    ...
    >
      {/* 立ち絵（大） */}
      <div className="relative mb-2" style={{ width: 84, height: 84 }}>
        {standUrl ? (
          <Image
            src={standUrl}
            ...
            sizes="84px"
            style={{ filter: dead ? 'grayscale(1)' : 'drop-shadow(0 4px 12px rgba(0,0,0,0.6))' }}
          />
        ) : (
      ...
      {/* 名前 */}
      <p className="mb-1.5 max-w-[88px] truncate text-center text-[10px] font-bold text-white/80">
        {actor.displayName}
      </p>

      {/* HP バー */}
      <div className="w-full overflow-hidden rounded-full bg-white/10" style={{ height: 5 }}>
        <div
      ...
      <p className="mt-0.5 text-[9px] text-white/35">
        {actor.currentHp} <span className="text-white/20">/ {actor.maxHp}</span>
      </p>

      {/* バフ表示 */}
      {(actor.buffTurnsRemaining > 0 || actor.shieldHitsRemaining > 0) && (
        <div className="mt-0.5 flex justify-center gap-0.5">
          {actor.atkMultiplier > 1  && <span className="text-[8px] text-yellow-400">↑ATK</span>}
          {actor.defMultiplier > 1  && <span className="text-[8px] text-blue-300">↑DEF</span>}
          {actor.atkMultiplier < 1  && <span className="text-[8px] text-red-400">↓ATK</span>}
          {actor.defMultiplier < 1  && <span className="text-[8px] text-orange-400">↓DEF</span>}
          {actor.shieldHitsRemaining > 0 && (
            <span className="text-[8px] text-cyan-400">🛡{actor.shieldHitsRemaining}</span>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        border:     actor.isMain ? '2px solid rgba(255,255,255,0.25)' : '1px solid rgba(255,255,255,0.08)',
...
export function BattleScreenPatternB({ battleState, isRunning, onSkillSelect }: BattleScreenProps) {
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(180deg, ${theme.bgTop} 0%, ${theme.bgBottom} 55%, #0a0f1e 100%)`,
      }}
    >

      {/* ══ ① ヘッダー ══ */}
      <header className="shrink-0 flex items-center justify-between px-4 py-2.5" style={{ background: theme.headerBg }}>
      ...
      {/* ══ ② 敵ショーケース ══ */}
      <section className="flex flex-1 flex-col items-center justify-center px-4 py-3">

        {/* 敵合算 HP バー */}
        {enemies.length > 0 && (
          <div className="mb-3 w-52">
            <div className="mb-1 flex items-center justify-between text-[9px]">
              <span className="font-bold text-red-400/70">ENEMY HP</span>
              <span className="text-white/30">{totalCurHp} / {totalMaxHp}</span>
            </div>
        ...
        <div className="flex items-end justify-center gap-6">
          {enemies.map((e) => (
            <EnemyCard key={e.id} actor={e} accent={theme.accent} />
          ))}
        </div>

        {/* フローティングバトルログ */}
        <div className="mt-4 w-full space-y-0.5 px-2">
          {lastLog.map((entry, i) => (
            <p
              key={`${entry.tick}-${i}`}
              className="text-center text-xs leading-5"
              style={{ opacity: lastLog.length > 1 && i === 0 ? 0.38 : 0.82 }}
            >
      ...
      {/* ══ ④ スキルボタン（大・グロー） ══ */}
      <section className="shrink-0 px-4 pb-5 pt-2">
        {mainSkills.length === 0 ? (
